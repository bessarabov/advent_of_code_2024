...
sub calculate_sides {
	...
	{
		my %tmp;
		foreach my $x_y (keys %{$edge}) {
			my ($x, $y) = split /_/, $x_y;
			if (not exists $all->{$x . '_' . ($y - 1)}) {
				$tmp{$y}->{$x} = 1;
			}
		}
		foreach my $y (keys %tmp) {
			$top_sides_count += count_conntected(keys %{$tmp{$y}});
		}
	}
...
#	p $top_sides_count;
...

	{
		...
		foreach my $y (keys %tmp) {
			$bottom_sides_count += count_conntected(keys %{$tmp{$y}});
		}
	}

#	p 'bottom_sides_count:';
#	p $bottom_sides_count;
...

	{
		...
		foreach my $x_y (keys %{$edge}) {
			my ($x, $y) = split /_/, $x_y;
			if (not exists $all->{($x - 1) . '_' . $y}) {
				$tmp{$x}->{$y} = 1;
			}
		}
		foreach my $x (keys %tmp) {
			$left_sides_count += count_conntected(keys %{$tmp{$x}});
		}
	}
...
#	p $left_sides_count;
...

	{
		...
		foreach my $x (keys %tmp) {
			$right_sides_count += count_conntected(keys %{$tmp{$x}});
		}
	}
...
#	p $right_sides_count;
    ...
    return $top_sides_count + $bottom_sides_count + $left_sides_count + $right_sides_count;;
}

sub count_conntected {
	my (@arr) = @_;
...
sub are_near {
    ...
    foreach my $x_y (keys %{$a1}) {
        my ($x, $y) = split /_/, $x_y;

        return 1 if exists $a2->{ ($x - 1) . '_' . $y };
        return 1 if exists $a2->{ ($x + 1) . '_' . $y };

        return 1 if exists $a2->{ $x . '_' . ($y - 1) };
        return 1 if exists $a2->{ $x . '_' . ($y + 1) };
    }
...
#    p \@field;
    ...
    while (1) {
        my $merged_clusters = 0;

        TMP:
        foreach my $c1 (sort keys %{$h}) {

            A:
            foreach my $c2 (sort keys %{$h}) {
                next A if $c2 == $c1;
...
#                };
                ...
                next A if ($h->{$c1}->{value} // '') ne ($h->{$c2}->{value} // '');
                if (are_near($h->{$c1}->{all_fields}, $h->{$c2}->{all_fields})) {

                    foreach my $x_y (keys %{$h->{$c2}->{all_fields}}) {
                        $h->{$c1}->{all_fields}->{$x_y} = 1;
                        $x_y2cluster_id->{$x_y} = $c1;
                    }
        ...
        last if $merged_clusters == 0;
    }

	foreach my $t1 (keys %{$h}) {
		my $tmp = $h->{$t1};
		if (keys(%{$tmp}) == 0) {
			delete $h->{$t1};
		}
...
#    p $x_y2cluster_id;
    ...
    foreach my $cluster_id (keys %{$h}) {
        $h->{$cluster_id}->{sides} = calculate_sides($h->{$cluster_id}->{all_fields});
...
#    p $h;
    ...
    foreach my $cluster_id (keys %{$h}) {
        $answer += $h->{$cluster_id}->{sides} * $h->{$cluster_id}->{area};
    }
